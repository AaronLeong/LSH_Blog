...
export default class extends Base {
    ...
    async indexAction() {
        ...
        //判断浏览客户端
        //判断浏览客户端
        if (checkIpad(this.userAgent())) {
            this.assign("retdata", '')
            return this.display(`format/jsonformat.html`)
        } else if (checkMobile(this.userAgent())) {
            this.redirect("/user/mobileindex")
        } else {
            return this.display();
        }
    }
    ...
    async mobileindexAction() {
        ...
        this.assign("userInfo", userInfo);
        formatData.userInfo = userInfo
        let avatar = await this.model("wx_user").where({uid: this.user.uid}).field("headimgurl").find();
        //console.log(avatar.headimgurl)
        this.assign("avatar", avatar.headimgurl);
        //提现总金额
        let cash = await this.model("cash").where({user_id: this.user.uid, status: 3}).getField('money');
        let cashTotal = eval(cash.join("+"));
        this.assign("cashTotal", cashTotal);
        formatData.cashTotal = cashTotal;

        //下级会员总数
        let recommended = await this.model("member_relation").where({user_id: this.user.uid}).count();
        this.assign('recommended', recommended);
        formatData.recommended = recommended
        ////console.log(userInfo);

        this.meta_title = "用户中心";
        //判断浏览客户端
        if (checkIpad(this.userAgent())) {
            this.assign("retdata", jsonFormat(formatData))
            return this.display(`format/jsonformat.html`)
        } else if (checkMobile(this.userAgent())) {
    ...
    async withdrawAction() {
        ...
        //
        //
        // //console.log('page', this.param('page'), this.get('page'));
        // // this.config("db.nums_per_page",20)
        // let data = await this.model("contract").where(map).page(this.get('page')).order("create_time DESC").countSelect();
        // let html = pagination(data, this.http, {
        //     desc: false, //show description
        //     pageNum: 2,
        //     url: '', //page url, when not set, it will auto generated
        ...
        // });
        // this.assign('pagination', html);
        // formatData.pagination = html
        //
        // //未处理
        // let nocheck = await this.model("contract").where({
        //     status: 1,
        //     user_id: this.user.uid,
        // }).count("id");
        //
        // this.assign("nocheck", nocheck);
        // formatData.nocheck = nocheck
        // //正在审核统计
        // let checking = await this.model("contract").where({
        //     status: 2,
        //     user_id: this.user.uid,
        // }).count("id");
        // this.assign("checking", checking);
        // formatData.checking = checking
        // //审核通过
        // let pass = await this.model("contract").where({
        //     status: 3,
        //     user_id: this.user.uid,
        // }).count("id");
        // this.assign("pass", pass);
        // formatData.pass = pass
        // //审核未通过
        // let unpass = await this.model("contract").where({
        //     status: 0,
        //     user_id: this.user.uid,
        // }).count("id");
        // this.assign("unpass", unpass);
        // formatData.unpass = unpass
        //
        //
        // ////console.log(data.data);
        // this.assign('list', data.data);
        // formatData.data = data.data
        // this.meta_title = "我的合同";
        // //判断浏览客户端
        if (checkIpad(this.userAgent())) {
            this.assign("retdata", jsonFormat(formatData))
            return this.display(`format/jsonformat.html`)
    ...
    async commissionAction() {
        ...
        ////console.log(map);
        // this.config("db.nums_per_page",20)
        let data = await this.model("commission").where(map).page(this.param('page')).order("create_time DESC").countSelect();
        let html = pagination(data, this.http, {
            desc: false, //show description
            pageNum: 2,
            url: '', //page url, when not set, it will auto generated
        ...
        ////未处理
        //let nocheck = await this.model("commission").where({
        //    status: 1,
        //    user_id: this.user.uid,
        //}).count("id");
        //
        ...
        //formatData.nocheck = nocheck
        ////正在审核统计
        //let checking = await this.model("commission").where({
        //    status: 2,
        //    user_id: this.user.uid,
        //}).count("id");
        //this.assign("checking", checking);
        //formatData.checking = checking
        ////审核通过
        //let pass = await this.model("commission").where({
        //    status: 3,
        //    user_id: this.user.uid,
        //}).count("id");
        //this.assign("pass", pass);
        //formatData.pass = pass
        ////审核未通过
        //let unpass = await this.model("commission").where({
        //    status: 0,
        //    user_id: this.user.uid,
        //}).count("id");
        //this.assign("unpass", unpass);
    ...
    async cashAction() {
        ...
        //判断浏览客户端
        if (checkIpad(this.userAgent())) {
            this.assign("retdata", "直接返回模板")
            return this.display(`format/jsonformat.html`)
        } else if (checkMobile(this.userAgent())) {
            if (this.isAjax("POST")) {
                return this.success('直接返回模板');
            } else {
                this.active = "user/index";
                return this.display(`mobile/${this.http.controller}/${this.http.action}`)
            }
     ...
     *申请提现 post
     * @returns {Promise}
     * http://127.0.0.1:8888/user/applycash/money/1
     */

    async applycashAction(self) {
        await this.weblogin();
        if (this.http.isPost()) {
            let postData = this.http._post
            let applycash = this.param('money')
            //let applycash = this.get('money')
            let user_id = this.user.uid;
            postData.user_id = this.user.uid;
            postData.create_time = timestamp();
            let userInfo = await this.model("customer").where({user_id: this.user.uid}).find();
            if (think.isNumberString(applycash)) {
                if (parseInt(applycash) <= userInfo.money) {
                    //console.log('是')
                    let money = userInfo.money - parseInt(applycash)
                    var model = this.model("cash");
                    let result = null;
                    let res = await model.transaction(function () {
                        return model.add(postData).then(function () {
                            return self.model("customer").where({user_id: user_id}).update({money: money});
                        });
                    }).then(function () {
                        ////console.log('成功了哦--------------')
                        result = 1
                    ...
                    if (result == 1) {
                        ...
                        return this.fail('系统错误,请稍后重试')
                    }
                } else {
                    //console.log('否')
                    return this.fail('提现金额不得高于余额')
                }
            } else {
                return this.fail('金额格式不正确')
            }
            this.assign("userInfo", userInfo);
        } else {
            return this.fail('请求用post')
        }
    }


    //提现明细
    async cashlistAction() {
        //判断是否登陆
        await this.weblogin();
        let status = this.param("status") || null;
        ////console.log(status);
        ...
        ////console.log(map);
        // this.config("db.nums_per_page",20)
        let data = await this.model("cash").where(map).page(this.param('page')).order("create_time DESC").countSelect();
        let html = pagination(data, this.http, {
            desc: false, //show description
            pageNum: 2,
            url: '', //page url, when not set, it will auto generated
        ...
        //未处理
        let nocheck = await this.model("cash").where({
            status: 1,
            user_id: this.user.uid,
        }).count("id");
        ...
        formatData.nocheck = nocheck
        //正在审核统计
        let checking = await this.model("cash").where({
            status: 2,
            user_id: this.user.uid,
        }).count("id");
        this.assign("checking", checking);
        formatData.checking = checking
        //审核通过
        let pass = await this.model("cash").where({
            status: 3,
            user_id: this.user.uid,
        }).count("id");
        this.assign("pass", pass);
        formatData.pass = pass
        //审核未通过
        let unpass = await this.model("cash").where({
            status: 0,
            user_id: this.user.uid,
        }).count("id");
        this.assign("unpass", unpass);
...
//   用户设置
    ...
    async registerAction() {
        if (this.isPost()) {
            ...
            return this.success({name: "注册成功,登录中!", url: "/user/mobileindex"});
        } else {
...
// //console.log(list);
    ...
    async collectAction() {
        ...
        //判断是否登陆
        await this.weblogin();
        let doc_id = this.param("doc_id");


        if (think.isEmpty(doc_id)) {
            //console.log('doc_id为空')
            return this.fail(1000, "参数错误,doc_id为空");
        } else {
            let map;
            //筛选订单
            map = {
            ...
            let collect = await this.model("doc_collection").where(map).select();

            if ((!think.isEmpty(collect)) && (collect[0].doc_id == doc_id)) {
                let iscollect = collect[0]

                ////console.log('iscollect', iscollect)
                ////console.log('collect[0]', collect[0])
                ////console.log('collect', collect)

                let update = {}

                //console.log('iscollectdoc_id', iscollect.doc_id)
                //console.log('iscollect', iscollect.status)

                if (iscollect.status == 0) {
                    update.status = 1;
                } else {
                    update.status = 0;
                }

                let res = await this.model("doc_collection").where(map).update(update);
                if (update.status == 0) {
                    //console.log('取消收藏update', update, '------------')
                    return this.success('取消收藏')
                } else {
                    //console.log('收藏成功update', update, '------------')
                    return this.success('收藏成功')
                }
            } else {
                map.status = 1;
                map.create_time = timestamp()
                let data = await this.model("doc_collection").add(map);
                if (data) {
                    return this.success('收藏成功')
                } else {
                    return this.fail(1000, "系统错误,请稍后重试");
                }
    ...
    async collectionAction() {
        ...
        //console.log(map);
        // this.config("db.nums_per_page",20)
        let data = await this.model("doc_collection").where(map).page(this.param('page')).order("create_time DESC").countSelect();
        ...
        ////console.log(this.setup.ORDER_DELAY_BUND)
        //查出收藏列表
        let collectList = await this.model("doc_collection").where(map).select();
        this.assign('collectList', collectList);
        formatData.collectList = collectList
        //收藏统计
        let collectCount = await this.model("doc_collection").where(map).count();
        this.assign("collectCount", collectCount);
        formatData.collectCount = collectCount
        //this.assign('list', data.data);
