...
export default class extends Base {
    ...
    async listAction() {
        ...
        let categories = await this.model('categories').page(this.get("page"), 10).countSelect();

        let html = pagination(categories, this.http, {
    ...
    async itemAction() {
        ...
        console.log('id', id)
        let res = await this.model('categories').where({'name': 'categories'}).find();
        console.log('res', res)
    ...
    async delectAction() {
        //auto render template file index_index.html
        let id = this.http.get('id')
        let delect = await this.model('categories').where({_id: id}).delete();
    ...
    async updateAction() {
        ...
        if (this.isPost()) {
            ...
            if (think.isEmpty(data._id)) {
                ...
                data.sys_name = "categories"
                let res = await this.model('categories').add(data)
                ...
                let res = await this.model('categories').where({_id: data._id}).update(data)
